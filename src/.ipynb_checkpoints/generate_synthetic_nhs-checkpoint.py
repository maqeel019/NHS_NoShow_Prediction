...
# Uses ONLY real patterns from NHS datasets - no arbitrary random features
...
# 1️⃣ LOAD NHS-DERIVED INPUTS

print("Loading NHS datasets...")
imd = pd.read_csv("./data/processed/imd_no_show_probs.csv", header=0)
ethnicity = pd.read_csv("./data/processed/ethnicity_no_show_probs.csv", header=0)
...
ethnicity = ethnicity[~ethnicity['Ethnicity'].isin(['Total Activity', 'Not Known'])]
...
age_gender['Main_Specialty_Desc'] = age_gender['Main_Specialty_Desc'].fillna('General Medicine')


# 3️⃣ CREATE PROBABILITY DISTRIBUTIONS
...
age_gender_counts = age_gender.groupby(['Age_Group', 'Gender'])['Attendance_Count'].sum()
age_gender_probs = age_gender_counts / age_gender_counts.sum()


# 4️⃣ BUILD JOINT PROBABILITY TABLE (without specialty yet)
...
joint_df['joint_prob'] = (
    ...
    joint_df.apply(lambda x: age_gender_probs.loc[(x['Age_Group'], x['Gender'])], axis=1)
)
...
sampled = joint_df.sample(n=n_patients, weights='joint_prob', replace=True).reset_index(drop=True)


# 6️⃣ ASSIGN MEDICAL SPECIALTIES (from real age/gender distributions)
...
specialty_dist = age_gender.groupby(['Age_Group', 'Gender', 'Main_Specialty_Desc'])['Attendance_Count'].sum().reset_index()
specialty_dist['specialty_prob'] = specialty_dist.groupby(['Age_Group', 'Gender'])['Attendance_Count'].transform(lambda x: x / x.sum())

def assign_specialty(age, gender):
    """Assign specialty based on real NHS age/gender distribution"""
    subset = specialty_dist[(specialty_dist['Age_Group'] == age) & (specialty_dist['Gender'] == gender)]
    if len(subset) == 0:
        return 'General Medicine'
    return np.random.choice(subset['Main_Specialty_Desc'].values, p=subset['specialty_prob'].values)

sampled['Medical_Specialty'] = [assign_specialty(a, g) for a, g in zip(sampled['Age_Group'], sampled['Gender'])]


# 7️⃣ USE REAL TELECONSULTATION PROPORTIONS
...
sampled['Consultation_Type'] = [
    np.random.choice(['Telemedicine', 'Face-to-Face'], p=[tp, 1-tp]) 
    for tp in sampled['Tele_Prob']
]


# 8️⃣ ASSIGN APPOINTMENT TYPE (First vs Subsequent) - from real data
...
# 9️⃣ CALCULATE BASE NO-SHOW PROBABILITY (from real data)
...
# 🔟 APPLY REAL-DATA-DRIVEN ADJUSTMENTS

print("Applying evidence-based adjustments...")

# Age adjustment (derived from NHS patterns - younger = higher no-show)
...
# Teleconsultation reduces no-show (evidence-based)
tele_adjust = {'Telemedicine': -0.02, 'Face-to-Face': 0.0}
...
# 1️⃣1️⃣ PATIENT HISTORY (based on adjusted probability)

print("Generating patient appointment history...")

sampled['Previous_Appointments'] = np.random.poisson(lam=3.5, size=n_patients).clip(0, 20)
...
# 1️⃣2️⃣ GENERATE ACTUAL NO-SHOW OUTCOME
...
# 1️⃣4️⃣ SAVE AND REPORT

final_df.to_csv("./data/processed/nhs_no_show_enhanced_synthetic.csv", index=False)

print("\n" + "="*70)
print("✅ ENHANCED SYNTHETIC NHS DATASET GENERATED!")
print("="*70)
# print(f"Total Patients: {len(final_df):,}")
# print(f"No-Show Rate: {final_df['NoShow'].value_counts(normalize=True)['Yes']:.2%}")
# print(f"\n📊 Feature Summary:")
# print(f"  - Medical Specialties: {final_df['Medical_Specialty'].nunique()}")
# print(f"  - Teleconsultation Rate: {(final_df['Consultation_Type']=='Telemedicine').mean():.2%}")
# print(f"  - First Appointments: {(final_df['Appointment_Type']=='First').mean():.2%}")
...
# print(final_df.groupby('Consultation_Type')['NoShow'].apply(lambda x: (x=='Yes').mean()).to_string())
# print(final_df.groupby('Appointment_Type')['NoShow'].apply(lambda x: (x=='Yes').mean()).to_string())